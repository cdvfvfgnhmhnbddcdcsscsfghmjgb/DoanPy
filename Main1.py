...
def add_item():
    ...
    tk.Button(add_window, text="Lưu", font=('Arial', 12), bg=button_color, fg=text_color, command=save_new_note).pack(pady=10)
...
def edit_item():
    ...
    if selected_index:
        ...
        tk.Button(edit_window, text="Lưu", font=('Arial', 12), bg=button_color, fg=text_color, command=save_edited_note).pack(pady=10)
...
# Hàm sắp xếp ghi chú theo ngày giờ
...
def create_menu():
    ...
    settings_menu.add_command(label="Thay đổi màu nút", command=change_button_color)
...
def change_button_color():
    global button_color, text_color
    color_code = colorchooser.askcolor(title="Chọn màu nút")[1]
    if color_code:
        button_color = color_code
        for button in [add_button, edit_button, delete_button, delete_permanently_button, delete_all_notes_button]:
...
add_button = tk.Button(root, text='Thêm Ghi Chú', bg=button_color, fg=text_color, font=('Times New Roman', 12), command=add_item)
add_button.place(x=41, y=120)

edit_button = tk.Button(root, text='Sửa Ghi Chú', bg=button_color, fg=text_color, font=('Times New Roman', 12), command=edit_item)
edit_button.place(x=46, y=164)

delete_button = tk.Button(root, text='Xóa Ghi Chú', bg=button_color, fg=text_color, font=('Times New Roman', 12), command=delete_item)
delete_button.place(x=46, y=208)

delete_permanently_button = tk.Button(root, text='Xóa Vĩnh Viễn', bg=button_color, fg=text_color, font=('Times New Roman', 12), command=delete_permanently)
delete_permanently_button.place(x=39, y=292)

delete_all_notes_button = tk.Button(root, text='Xóa Tất Cả Ghi Chú', bg=button_color, fg=text_color, font=('Times New Roman', 12), command=delete_all_notes)
delete_all_notes_button.place(x=21, y=334)

tk.Button(root, text='Thoát', bg='red', fg='white', font=(14), command=root.quit).place(x=65, y=376)
